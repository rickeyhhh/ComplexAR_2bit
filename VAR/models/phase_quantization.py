...
class QATLinearComplexReorder(nn.Linear):
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        # Backward compatibility: old checkpoints do not contain cached reorder permutation.
        ...
        # Forward compatibility: allow loading checkpoints where cached perms
        # were materialized (non-empty) while current module starts from empty.
    ...
    def _build_reorder_perm(self, A: torch.Tensor) -> torch.Tensor:
        ...
        # Sort by absolute row score, then place ranks alternately into upper/lower halves.
    ...
    def _get_cached_permutation(self, A: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if need_rebuild:
            ...
            print(f'  [reorder QAT] built new perm: out_features={A.shape[0]}')
    ...
    def forward(self, x):
        ...
        # Reorder rows and run the Triton quantization kernel on the whole
        # reordered weight matrix. Use fake-quant pattern to preserve gradients:
        #   A_quant_reordered = (kernel(A_reordered) - A_reordered).detach() + A_reordered
        A_reordered = A[row_perm, :]

        # Triton kernel returns bfloat16 quantized matrix
        B_quant_bf16 = fairytoi_quant_block_V2(A_reordered)
        # Cast back to original dtype and preserve gradient via residual detach trick
...
def replace_modules_for_qat(
    ...
    skip_head_nm: bool = False,
    skip_word_embed: bool = False,
    skip_ada_lin: bool = False,
):
    ...
    for name, module in model.named_children():
        ...
        if skip_head_nm and name == 'head_nm':
            print(f"  -> Skipping head_nm module: {name}")
            continue

        if skip_word_embed and name == 'word_embed':
            print(f"  -> Skipping word_embed module: {name}")
            continue

        if skip_ada_lin and name in ('shared_ada_lin', 'ada_lin'):
            print(f"  -> Skipping ada linear module: {name}")
            continue
        logging.info(f"name: {name}, module: {module}")

        if len(list(module.children())) > 0:
            replace_modules_for_qat(module, method, skip_head, skip_head_nm, skip_word_embed, skip_ada_lin)

        if isinstance(module, nn.Linear):
            if module.in_features % 2 != 0 or module.out_features % 2 != 0:
                print(f"  -> Skipping Complex replacement (non-even dimensions): {name} ({module.in_features}, {module.out_features})")
                continue
...
class InferenceOptimizedComplexReorder(nn.Linear):
    ...
    def __init__(self, version="reorder", *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self.in_features % 2 != 0 or self.out_features % 2 != 0:
            raise ValueError("Complex-Reorder requires even in/out features.")
    ...
    @torch.no_grad()
    def set_reorder_permutation(self, row_perm: torch.Tensor, row_inv: torch.Tensor):
        if row_perm.numel() != self.out_features or row_inv.numel() != self.out_features:
            raise ValueError("reorder permutation shape mismatch")
        ...
        if not self._is_quantized:
            with torch.no_grad():
                ...
                if self._row_perm.numel() == A.shape[0] and self._row_inv.numel() == A.shape[0]:
                    ...
                    print(f'  [reorder infer] using loaded perm: out_features={A.shape[0]}')
                else:
                    row_perm = self._build_reorder_perm(A)
                    row_inv = torch.empty_like(row_perm)
                    row_inv[row_perm] = torch.arange(row_perm.numel(), device=row_perm.device)
                    self._row_perm = row_perm
                    self._row_inv = row_inv
                    print(f'  [reorder infer] built new perm (no cached): out_features={A.shape[0]}')
        ...
        for name, child in list(module.named_children()):
            ...
            if isinstance(child, QATLinearComplexReorder):
                ...
                if row_perm.numel() == child.out_features and row_inv.numel() == child.out_features:
                    new_module.set_reorder_permutation(row_perm, row_inv)

                setattr(module, name, new_module)
                converted_count += 1
                print(f"  -> Converting Reorder layer: {full_name}")
            else:
