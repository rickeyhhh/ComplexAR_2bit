...
def build_everything(args: arg_util.Args):
    qat_method = 'complex_phase_v2_reorder_2'
    args.qat_method = qat_method

    # resume
    print(f"[Rank {dist.get_rank()}] Start auto_resume...", flush=True)
    auto_resume_info, start_ep, start_it, trainer_state, args_state = auto_resume(args, 'ar-ckpt*.pth')
    print(f"[Rank {dist.get_rank()}] Finish auto_resume...", flush=True)

    # create tensorboard logger
    tb_lg: misc.TensorboardLogger
    with_tb_lg = dist.is_master()
    if with_tb_lg:
        print(f"[Rank {dist.get_rank()}] Making log dir...", flush=True)
        os.makedirs(args.tb_log_dir_path, exist_ok=True)
        print(f"[Rank {dist.get_rank()}] Init Tensorboard...", flush=True)
        ...
        tb_lg.flush()
        print(f"[Rank {dist.get_rank()}] Master Ready!", flush=True)
    else:
        tb_lg = misc.DistLogger(None, verbose=False)
        print(f"[Rank {dist.get_rank()}] Worker Ready!", flush=True)
    ...
    ### load pretrained VAR checkpoint
    var_ckpt = '
    print(f'[INIT] Loading pretrained VAR model from {var_ckpt}')

    ckpt = torch.load(var_ckpt, map_location='cpu')
    if isinstance(ckpt, dict) and isinstance(ckpt.get('trainer', None), dict) and 'var_wo_ddp' in ckpt['trainer']:
        var_state_dict = ckpt['trainer']['var_wo_ddp']
    elif isinstance(ckpt, dict) and 'var_wo_ddp' in ckpt:
        var_state_dict = ckpt['var_wo_ddp']
    else:
        var_state_dict = ckpt

    if not isinstance(var_state_dict, dict):
        raise RuntimeError(f'[INIT] Unsupported checkpoint format in {var_ckpt}')

    # Handle common wrappers from DDP/trainer saves.
    if any(k.startswith('module.') for k in var_state_dict.keys()):
        var_state_dict = {k[7:] if k.startswith('module.') else k: v for k, v in var_state_dict.items()}
    if any(k.startswith('var_wo_ddp.') for k in var_state_dict.keys()):
        var_state_dict = {k[11:] if k.startswith('var_wo_ddp.') else k: v for k, v in var_state_dict.items()}

    var_wo_ddp.load_state_dict(var_state_dict, strict=True)

    ### replace QAT modules (reorder2)
    from models.phase_quantization import replace_modules_for_qat
    ...
    print(f"VAR modules replaced with QAT layers successfully! method={qat_method}")

    vae_local: VQVAE = args.compile_model(vae_local, args.vfast)
    var_wo_ddp: VAR = args.compile_model(var_wo_ddp, args.tfast)
    # Reorder2 QAT may leave some quantizer parameters unused in some iterations,
    # so enable unused-parameter detection in DDP.
    ...
    names, paras, para_groups = filter_params(var_wo_ddp, nowd_keys=nowd_keys)
    opt_clz = {
    ...
    for ep in range(start_ep, args.ep):
        ...
        if is_val_and_also_saving:
            ...
            if dist.is_master():
                ...
                # Save an epoch-specific copy that won't be overwritten by future checkpoints
                local_out_ckpt_ep = os.path.join(args.local_out_dir_path, f'ar-ckpt-ep{ep+1}.pth')
                ...
                # Save a copy with epoch suffix so it's preserved permanently
                shutil.copy(local_out_ckpt, local_out_ckpt_ep)

                if best_updated:
                    shutil.copy(local_out_ckpt, local_out_ckpt_best)
                print(f'     [saving ckpt](*) finished!  @ {local_out_ckpt} and {local_out_ckpt_ep}', flush=True, clean=True)
            dist.barrier()
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger, ld_or_itrt, iters_train: int, trainer):
    ...
    # for it, (inp, label) in me.log_every(start_it, iters_train, ld_or_itrt, 30, header):
